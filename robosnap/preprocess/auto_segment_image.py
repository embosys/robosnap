...
Every coordinate value in bbox_xyxy and geometry_prompts.points MUST be a decimal floating-point value in the inclusive range [0.0, 1.0]. Never output pixel coordinates and never use Gemini's 0-to-1000 coordinate convention. If coordinates were estimated on a 0-to-1000 scale, divide every coordinate by 1000 before returning JSON. Example bbox_xyxy: [0.10, 0.25, 0.80, 0.95]. Example point: [0.45, 0.60].
